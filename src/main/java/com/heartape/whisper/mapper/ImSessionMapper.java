...
public interface ImSessionMapper {
    List<ImSessionResult> selectListByUserId(Long userId);

    List<ImSessionWithMemberUser> selectPeerListWithUserByUserId(Long userId);

    List<ImMessage> messages(Integer count, Long since, Long before);

    List<ImMessage> messages(Long sessionId, Integer count, Long since, Long before);
